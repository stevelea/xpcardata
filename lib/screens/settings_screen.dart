...
class _SettingsScreenState extends ConsumerState<SettingsScreen> {
  ...
  /// Auto-save a boolean setting immediately when toggled
  Future<void> _autoSaveBool(String key, bool value) async {
    ...
    try {
      ...
      print('SharedPreferences auto-save failed for $key: $e');
    }
              ...
              child: Column(
                ...
                children: [
                  ...
                  SwitchListTile(
                    ...
                    subtitle: Text(
                      ref.watch(dataSourceManagerProvider).isLeftHandDrive
                          ? 'Speed on left, Battery on right (LHD countries)'
                          : 'Battery on left, Speed on right (RHD countries)',
                    ),
                    secondary: Icon(
                      Icons.swap_horiz,
                      color: ref.watch(dataSourceManagerProvider).isLeftHandDrive
                          ? Colors.blue
                          : Colors.grey,
                    ),
                    value: ref.watch(dataSourceManagerProvider).isLeftHandDrive,
                    onChanged: (value) async {
                      final manager = ref.read(dataSourceManagerProvider);
                      await manager.setLeftHandDrive(value);
                      setState(() {});
                    },
                  ),
                  Padding(
                    padding: const EdgeInsets.symmetric(horizontal: 16),
                    child: Text(
                      'For LHD countries (driver on left), important dashboard info is positioned closer to the driver.',
                      style: Theme.of(context).textTheme.bodySmall?.copyWith(
                        color: Theme.of(context).colorScheme.onSurfaceVariant,
                      ),
                    ),
