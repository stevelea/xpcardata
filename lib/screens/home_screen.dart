...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  Widget _buildServiceStatusRow(WidgetRef ref) {
    ...
    // Use battery current (HV_A) as the primary indicator - NEGATIVE = charging into battery
    // XPENG convention: negative current = energy flowing into battery (charging)
    // DC_CHG_A and AC_CHG_A return stale values when not charging, so they're unreliable
    bool isCharging = false;
    vehicleData.whenData((data) {
      if (data != null) {
        // Primary method: check battery current from BMS (HV_A mapped to batteryCurrent)
        ...
        // Also check additionalProperties for CHARGING PID or BMS_CHG_STATUS
        final props = data.additionalProperties ?? {};
        final chargingPid = props['CHARGING'];
        final bmsStatus = props['BMS_CHG_STATUS'];
        if (chargingPid != null && (chargingPid == 1 || chargingPid == 1.0)) {
          isCharging = true;
        }
        // BMS_CHG_STATUS: 2=DC, 3=AC, 4=DC high power
        if (bmsStatus != null && (bmsStatus == 2 || bmsStatus == 3 || bmsStatus == 4 ||
            bmsStatus == 2.0 || bmsStatus == 3.0 || bmsStatus == 4.0)) {
          isCharging = true;
        }
      }
    });
