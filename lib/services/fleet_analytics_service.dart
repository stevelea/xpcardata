...
class FleetAnalyticsService {
  ...
  Future<void> _loadSettings() async {
    ...
    try {
      ...
      prefsLoaded = true;
      _logger.log('[FleetAnalytics] SharedPrefs: enabled=$prefsEnabled, consent=$prefsConsent');
    } catch (e) {
    ...
    try {
      ...
      if (await file.exists()) {
        ...
        fileLoaded = true;
        _logger.log('[FleetAnalytics] File: enabled=$fileEnabled, consent=$fileConsent');
      }
    } catch (e) {
  ...
  Future<FleetStats?> _computeStatsFromContributions() async {
    ...
    try {
      ...
      _logger.log('[FleetAnalytics] Got ${allDocs.docs.length} total contributions');
      ...
      _logger.log('[FleetAnalytics] Computed stats: ${uniqueDevices.length} contributors, avg SOH: ${avgSoh.toStringAsFixed(1)}%');
